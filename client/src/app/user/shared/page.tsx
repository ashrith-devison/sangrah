...
export default function SharedFilesPage() {
  ...
  const getPermissionBadge = (permission: string) => {
    switch (permission) {
      case 'view':
        return <Badge variant="outline" className="border-blue-500 text-blue-400 text-xs">View</Badge>;
      case 'edit':
        return <Badge variant="outline" className="border-green-500 text-green-400 text-xs">Edit</Badge>;
      case 'admin':
        return <Badge variant="outline" className="border-purple-500 text-purple-400 text-xs">Admin</Badge>;
      default:
        return <Badge variant="outline" className="border-gray-500 text-gray-400 text-xs">Unknown</Badge>;
    }
  };
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      ...
                      <span className="truncate">
                        {sortBy === 'shared' ? 'Recently Shared' : 
                         sortBy === 'name' ? 'Name' : 
                         sortBy === 'size' ? 'Size' : 'Permissions'}
                      </span>
                      <ChevronDown className="w-4 h-4 ml-auto flex-shrink-0" />
                    </Button>
                  </DropdownMenuTrigger>
          ...
          <CardContent className="p-6">
                        ...
                        <div className="group bg-zinc-800/30 hover:bg-zinc-800/50 border border-zinc-700 rounded-xl p-4 cursor-pointer transition-all hover:border-[#6e73fa]/50">
                            ...
                            <div className="flex items-center gap-2">
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <ItemIcon className={`w-8 h-8 ${iconColor}`} />
                                </TooltipTrigger>
                                <TooltipContent className="bg-zinc-800 border-zinc-700">
                                  <p className="text-white">{item.type === 'folder' ? 'Shared folder' : `${item.fileType} file`}</p>
                                </TooltipContent>
                              </Tooltip>
                              {item.isPublic && (
                                <Tooltip>
                          ...
                          <div className="space-y-1 text-xs text-gray-400">
                            ...
                            <div className="flex items-center">
                              ...
                              <span className="truncate">by {item.sharedBy}</span>
                            </div>
                          </div>
                        </div>
                      </ContextMenuTrigger>
                      ...
                      <ContextMenuTrigger>
                        <div className="group flex items-center justify-between p-4 bg-zinc-800/30 hover:bg-zinc-800/50 border border-zinc-700 rounded-lg cursor-pointer transition-all hover:border-[#6e73fa]/50 mb-2">
                          <div className="flex items-center space-x-4 flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <ItemIcon className={`w-6 h-6 ${iconColor} flex-shrink-0`} />
                              {item.isPublic && (
            ...
            {filteredItems.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-white font-medium mb-2">No shared items found</h3>
